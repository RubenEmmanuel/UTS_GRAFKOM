...
var currentTime = Date.now();
  ...
  MODEL_MATRIX = LIBS.get_I4();
  LIBS.scale2(MODEL_MATRIX, 1, 1, 1);
...
var axis = [1, 0, 0]; // Rotating around the diagonal axis [1, 1, 0]
    var dt = time - time_prev;
    time_prev = time;
    ...
    LIBS.scale2(MODEL_MATRIX, scaleNow, 1, 1);
    //  LIBS.rotateArbitraryAxis(MODEL_MATRIX, angle, axis);
    LIBS.translate(MODEL_MATRIX, -spheres.x, 0, -spheres.z);
    LIBS.rotateY(MODEL_MATRIX, angle);
    LIBS.translate(MODEL_MATRIX, spheres.x, 0, spheres.z);


  // LIBS.scale2(MODEL_MATRIX,random);
  LIBS.translate(MODEL_MATRIX, -spheres.x, -spheres.y, -spheres.z)
    LIBS.rotateY(MODEL_MATRIX, theta);
    LIBS.rotateX(MODEL_MATRIX, alpha);
    LIBS.translate(MODEL_MATRIX, spheres.x, spheres.y, spheres.z)
...
//   scaleNow += scaleAdd;
// if(scaleNow >= 2) scaleAdd *= -1;
// else if(scaleNow <= 1) scaleAdd *= -1;
...
//z(- blkng)x (- to kanan)y - kebawah
  ...
  LIBS.scale2(MODEL_MATRIX, scaleNow*1.2, scaleNow*1.2, scaleNow*1.2);
...
var animatewalkingrest = function (time) {
 ...
 translatewalkingrest += translatewalkingrestspeed; //speed muter
  ...
  var warnaBadan3 = [255/255,109/255,109/255];//merah
  var warnaKuning3 = [255/255,218/255,161/255];//oren dikit
  var warnaTelinga3 = [153/255, 76/255, 0/255];//coklat
  var warnaMata3 = [153/255, 76/255, 0/255];//coklat x oren
  var warnaKaki3 = [204/255, 102/255, 0/255];

  // Badan
  var badanR = generateTubeM(-14, 0, 1, warnaBadan3[0], warnaBadan3[1], warnaBadan3[2], 3, 2, 2, 100);
  // Kepala
  var kepalaR = generateSphere(-11, 0, 1, warnaKuning3[0], warnaKuning3[1], warnaKuning3[2], 2, 100);
  // Belakang
  var belakangR = generateSphere(-14, 0, 1, warnaKuning3[0], warnaKuning3[1], warnaKuning3[2], -2, 100);
  // Telinga Kanan
  var telingaKananR = generateEllipsoid(-10.5, 1.5, 2.1, 0.2, 0.8, 0.6, warnaTelinga3[0], warnaTelinga3[1], warnaTelinga3[2], 100)
  // Telinga Kiri
  var telingaKiriR = generateEllipsoid(-10.6, 1.4, -0.3, 0.2, 0.8, 0.6, warnaTelinga3[0], warnaTelinga3[1], warnaTelinga3[2], 100)
  // Mata Satu
  var mataSatuR = generateEllipsoid(-9.1, 0.7, 1.5, 0.1, 0.4, 0.2, warnaMata3[0], warnaMata3[1], warnaMata3[2], 100)
  // Mata Dua
  var mataDuaR = generateEllipsoid(-9.1, 0.7, 0.5, 0.1, 0.4, 0.2, warnaMata3[0], warnaMata3[1], warnaMata3[2], 100)
  // Kaki 1
  var kakiSatuR = generateEllipticParaboloidR(10.5, 2, -2.3, 0.3, 0.3, warnaKaki3[0], warnaKaki3[1], warnaKaki3[2], 100);
  // Kaki 2
  var kakiDuaR = generateEllipticParaboloidR(10.5, 0, -2.3, 0.3, 0.3, warnaKaki3[0], warnaKaki3[1], warnaKaki3[2], 100);
  // Kaki 3
  var kakiTigaR = generateEllipticParaboloidR(14.2, 2, -2.3, 0.3, 0.3, warnaKaki3[0], warnaKaki3[1], warnaKaki3[2], 100);
  // Kaki 4
  var kakiEmpatR = generateEllipticParaboloidR(14.2, 0, -2.3, 0.3, 0.3, warnaKaki3[0], warnaKaki3[1], warnaKaki3[2], 100);
  // Hidung
  var hidungR = generateTubeM(-9, 0, 1, warnaTelinga3[0], warnaTelinga3[1], warnaTelinga3[2], 0.2, 0.2, 0, 100);
  ...
  //Badan
  var BADANR_VERTEX = createVertexBuffer(GL, badanR);
  var BADANR_COLORS = createColorBuffer(GL, badanR);
  var BADANR_FACES = createFacesBuffer(GL, badanR);


  //Kepala
  var KEPALAR_VERTEX = createVertexBuffer(GL, kepalaR);
  var KEPALAR_COLORS = createColorBuffer(GL, kepalaR);
  var KEPALAR_FACES = createFacesBuffer(GL, kepalaR);

  //Belakang
  var BELAKANGR_VERTEX = createVertexBuffer(GL, belakangR);
  var BELAKANGR_COLORS = createColorBuffer(GL, belakangR);
  var BELAKANGR_FACES = createFacesBuffer(GL, belakangR);

  //Telinga Kiri
  var TELINGA_KIRIR_VERTEX = createVertexBuffer(GL, telingaKiriR);
  var TELINGA_KIRIR_COLORS = createColorBuffer(GL, telingaKiriR);
  var TELINGA_KIRIR_FACES = createFacesBuffer(GL, telingaKiriR);

  //Telinga Kanan
  var TELINGA_KANANR_VERTEX = createVertexBuffer(GL, telingaKananR);
  var TELINGA_KANANR_COLORS = createColorBuffer(GL, telingaKananR);
  var TELINGA_KANANR_FACES = createFacesBuffer(GL, telingaKananR);

  //Mata Satu
  var MATASATUR_VERTEX = createVertexBuffer(GL, mataSatuR);
  var MATASATUR_COLORS = createColorBuffer(GL, mataSatuR);
  var MATASATUR_FACES = createFacesBuffer(GL, mataSatuR);

  //Mata Dua
  var MATADUAR_VERTEX = createVertexBuffer(GL, mataDuaR);
  var MATADUAR_COLORS = createColorBuffer(GL, mataDuaR);
  var MATADUAR_FACES = createFacesBuffer(GL, mataDuaR);

  //Kaki Satu
  var KAKI_SATUR_VERTEX = createVertexBuffer(GL, kakiSatuR);
  var KAKI_SATUR_COLORS = createColorBuffer(GL, kakiSatuR);
  var KAKI_SATUR_FACES = createFacesBuffer(GL, kakiSatuR);

  //Kaki Dua
  var KAKI_DUAR_VERTEX = createVertexBuffer(GL, kakiDuaR);
  var KAKI_DUAR_COLORS = createColorBuffer(GL, kakiDuaR);
  var KAKI_DUAR_FACES = createFacesBuffer(GL, kakiDuaR);

  //Kaki Tiga
  var KAKI_TIGAR_VERTEX = createVertexBuffer(GL, kakiTigaR);
  var KAKI_TIGAR_COLORS = createColorBuffer(GL, kakiTigaR);
  var KAKI_TIGAR_FACES = createFacesBuffer(GL, kakiTigaR);

  //Kaki Empat
  var KAKI_EMPATR_VERTEX = createVertexBuffer(GL, kakiEmpatR);
  var KAKI_EMPATR_COLORS = createColorBuffer(GL, kakiEmpatR);
  var KAKI_EMPATR_FACES = createFacesBuffer(GL, kakiEmpatR);

  //Hidung
  var HIDUNGR_VERTEX = createVertexBuffer(GL, hidungR);
  var HIDUNGR_COLORS = createColorBuffer(GL, hidungR);
  var HIDUNGR_FACES = createFacesBuffer(GL, hidungR);

  var time_prev = 0;


var animateR = function (time) {
  GL.viewport(0, 0, CANVAS.width, CANVAS.height);
  var dt = time - time_prev;
  time_prev = time;
  ...
  var currentTime = Date.now();
  var translateY = Math.sin(currentTime * 0.005 ) * 1;
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, BADANR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, BADANR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, BADANR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, badanR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, KEPALAR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, KEPALAR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, KEPALAR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, kepalaR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, BELAKANGR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, BELAKANGR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, BELAKANGR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, belakangR.faces.length, GL.UNSIGNED_SHORT, 0);

  GL.flush();
  window.requestAnimationFrame(animateR);
};

var animateDua = function (time) {
  GL.viewport(0, 0, CANVAS.width, CANVAS.height);
  var dt = time - time_prev;
  time_prev = time;
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, TELINGA_KIRIR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, TELINGA_KIRIR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, TELINGA_KIRIR_FACES);

  GL.drawElements(GL.TRIANGLES, telingaKiriR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, TELINGA_KANANR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, TELINGA_KANANR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, TELINGA_KANANR_FACES);

  GL.drawElements(GL.TRIANGLES, telingaKananR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, MATASATUR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, MATASATUR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, MATASATUR_FACES);

  GL.drawElements(GL.TRIANGLES, mataSatuR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, MATADUAR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, MATADUAR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, MATADUAR_FACES);

  GL.drawElements(GL.TRIANGLES, mataDuaR.faces.length, GL.UNSIGNED_SHORT, 0);

  GL.flush();
  window.requestAnimationFrame(animateDua);
};

  var time_prevR=0;
  var time_startR = Date.now();
  var maxTranslation = 1; // Maksimum jarak translasi dari pusat (dalam unit)
  var periodR = 4000; // Periode gerakan (dalam milidetik)
  var rotateKakiR = 0;
  var rotateSpeedR = 0.08;
  var kakiRangeR;
  var kakiAngleR;

  var animateKakiR = function (time) {
    GL.viewport(0, 0, CANVAS.width, CANVAS.height);
    var dt = time - time_prevR;
    time_prev = time;
    ...
    rotateKakiR += rotateSpeedR;

    kakiRangeR = Math.PI / 18;

    kakiAngleR = Math.sin(rotateKakiR) * kakiRangeR;
  ...
  MODEL_MATRIX = LIBS.get_I4();
        LIBS.translateY(MODEL_MATRIX, translateY);
        LIBS.rotateX(MODEL_MATRIX, Math.PI / -20);
        LIBS.rotateX(MODEL_MATRIX, kakiAngleR);
        LIBS.rotateY(MODEL_MATRIX, theta);
    LIBS.rotateX(MODEL_MATRIX, alpha);

  
  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_SATUR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_SATUR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, KAKI_SATUR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, kakiSatuR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_DUAR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_DUAR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, KAKI_DUAR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, kakiDuaR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_TIGAR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_TIGAR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, KAKI_TIGAR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, kakiTigaR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_EMPATR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, KAKI_EMPATR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, KAKI_EMPATR_FACES);

  GL.uniformMatrix4fv(_PMatrix, false, PROJECTION_MATRIX);
  GL.uniformMatrix4fv(_VMatrix, false, VIEW_MATRIX);
  GL.uniformMatrix4fv(_MMatrix, false, MODEL_MATRIX);

  GL.drawElements(GL.TRIANGLES, kakiEmpatR.faces.length, GL.UNSIGNED_SHORT, 0);
  
  GL.flush();
  window.requestAnimationFrame(animateKakiR);

  };

  var animateTiga = function (time) {
    GL.viewport(0, 0, CANVAS.width, CANVAS.height);
  ...
  GL.bindBuffer(GL.ARRAY_BUFFER, HIDUNGR_VERTEX);
  GL.vertexAttribPointer(_position, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ARRAY_BUFFER, HIDUNGR_COLORS);
  GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 0, 0);

  GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, HIDUNGR_FACES);

  GL.drawElements(GL.TRIANGLES, hidungR.faces.length, GL.UNSIGNED_SHORT, 0);
  ...
  drawBezierCurve(GL,SHADER_PROGRAM,mulut1,100);
  drawBezierCurve(GL,SHADER_PROGRAM,alis1,100);
  drawBezierCurve(GL,SHADER_PROGRAM,alis2,100);

  GL.flush();
  window.requestAnimationFrame(animateTiga);
};
